...
import { useGLTF, useAnimations, Environment, Html, useAspect, useVideoTexture, useTexture } from '@react-three/drei'
import { useCharacterAnimations } from "../contexts/CharacterAnimations";
import { Color, LinearFilter, VideoTexture, MeshStandardMaterial, MeshPhysicalMaterial, MeshLambertMaterial, ShaderMaterial, MeshBasicMaterial, MeshPhongMaterial, sRGBEncoding, SRGBColorSpace } from 'three';
...
const group = useRef();
//const ref = useRef();
const blueGroup = useRef();
const { nodes, animations } = useGLTF("./models/woman.gltf");
const { setAnimations, animationIndex } = useCharacterAnimations();
const {  envMapIntensity,  setEnvMapIntensity } = useState(0);
...
//var [envMapIntensity, setEnvMapIntensity] = useState(2); // Initial value, change as needed
//console.log(MotionBlurPass);
	
const video = document.createElement('video');
video.src = 'Idle.mp4';
video.loop = true; // Optional: Loop the video
video.style.width = '100px'; // Adjust this value to your desired width
video.style.height = '100px';
// Create a texture from the video
const videoTexture = new VideoTexture(video);
videoTexture.colorSpace  = SRGBColorSpace;
//videoTexture.needsUpdate = true;
videoTexture.minFilter = LinearFilter; // Set the minFilter as needed
...
// Ensure the video is playing
	 ...
	 // Set the envMapIntensity using the setEnvMapIntensity function
  //setEnvMapIntensity(envMapIntensity);

if (materials) {
	//console.log("tutu : " + props.envMapIntensity);
  // Remplacez 'Mat.006' par le nom de votre matériau si nécessaire
    materials['Mat.006'] = new MeshPhysicalMaterial({
	//	metalness: 0.0, // Valeur de métal entre 0 (non métallique) et 1 (entièrement métallique)
	//	roughness: 1.0, // Valeur de rugosité entre 0 (lisse) et 1 (rugueux)
		envMapIntensity: props.envMapIntensity,
		depthTest: true,
		depthWrite: true,
		thickness: 1.0,
		color: props.envMapColor,  //#03bafc #00FF00  #ffff00 #ff9900 #f94449 #ff00ff
		transmission: 0.0,
  });
	
	materials.visage1 = new MeshPhysicalMaterial( { 
		map: videoTexture,
		...
		visible: true,
		color: 0xffffff, 
		emissive: props.envMapColor, 

		emissiveMap: videoTexture,
		emissiveIntensity: props.envMapIntensity/4,
...
/*
	...
	//materials.visage1.map = videoTexture;
	materials.visage1.map.center.set(0.5, 0.5);
	materials.visage1.map.rotation = Math.PI;
	materials.visage1.map.scale = 1;
	//materials.visage1.emissive = 0x0000f0;
	//materials.visage1.map.alphaMap = videoTexture;
	materials.visage1.map.color = '#fff';
//	materials.visage1.map.hue = 2.0;
	materials.visage1.map.saturation = 50;
	//materials.visage1.map.color.setHSL( material.hue, material.saturation, 0.5 );
	
	
	materials['Mat.007'] = new MeshPhysicalMaterial({  
				envMapIntensity: (props.envMapIntensity - 8),
		depthTest: true,
		depthWrite: true,
		thickness: 1.0,
		color: props.envMapColor, //#03bafc //#03bafc //#00FF00
		transmission: 0.0,
  });
	//console.log("ooooooo : " + props.envHdr)

	materials['LIGHTS_BODY.002'] = new MeshStandardMaterial({
		metalness: 0.5, // Valeur de métal entre 0 (non métallique) et 1 (entièrement métallique)
		roughness: 0.3, // Valeur de rugosité entre 0 (lisse) et 1 (rugueux)
		color: '#ffffff',
	});

	materials['materials.Matériau'] = new MeshStandardMaterial({
	...
	// Function to set envMapIntensity for the 'Mat.006' material
const setEnvMapIntensityForMaterial = (value) => {
  if (materials && materials['Mat.006']) {
	  //console.log("valueW : " + props.envMapIntensity);
	  materials['Mat.006'].envMapIntensity = props.envMapIntensity;
	  console.log("ttttttttt : " + value)
	 // materials['Mat.006'].color = props.envMapColor;
   // materials['Mat.006'].envMapIntensity = value;
	  // setEnvMapIntensity(value);
  }
};

// Update the envMapIntensity based on the state variable
useEffect(() => {
  setEnvMapIntensityForMaterial(envMapIntensity);
	console.log("yeezz1 : " + props.envMapIntensity);
}, [props.envMapIntensity]);


	
const setEnvMapColorForMaterial = (value) => {
  if (materials && materials['Mat.006']) {
	  console.log("xxxxxxx : " + props.envMapColor);
	  materials['Mat.006'].roughness = props.envMapColor;
	 // materials['Mat.006'].color = props.envMapColor;
   // materials['Mat.006'].envMapIntensity = value;
	  // setEnvMapIntensity(value);
  }
	...
	useEffect(() => {
  setEnvMapColorForMaterial(envMapColor);
	console.log("yeezz2 : " + props.envMapColor);
}, [props.envMapColor]);
  ...
  useEffect(() => {
	   ...
	   // console.log("names: ", names);
        console.log("animationIndexW: ", props.envMapIntensity);
...
//	    <Bloom  kernelSize={2} luminanceThreshold={0.0} luminanceSmoothing={0.8} intensity={0.4} />
	 
 return (
	<>
      <EffectComposer disableNormalPass>
        <DepthOfField focusDistance={0} focalLength={0.2} bokehScale={400} height={640} />
           <Bloom selection={blueGroup.current} intensity={0.4} luminanceThreshold={0.0} luminanceSmoothing={0.0}  />
      </EffectComposer>
        ...
        <group  name="FLUO" position={[0, 0.579, 0]} rotation={[-1.571, 1.55, 1.571]} scale={0.005}>
          <group name="AVATAR_ROTATION">
              ...
              <group name="base" position={[0, 1.527, 0]}>
                ...
                <group name="Null_31_2" position={[21.086, 0, -25.385]} rotation={[0.504, -0.629, 0.314]}>
                  <group name="Cube_1_instance_130" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4031" geometry={nodes.Cube_1_4031.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                ...
                <group name="Null_36_2" position={[23.787, 0, -22.874]} rotation={[0.549, -0.725, 0.386]}>
                  <group name="Cube_1_instance_135" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4036" geometry={nodes.Cube_1_4036.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_37_2" position={[24.292, 0, -22.336]} rotation={[0.56, -0.744, 0.402]}>
                  <group name="Cube_1_instance_136" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4037" geometry={nodes.Cube_1_4037.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_38_2" position={[24.785, 0, -21.787]} rotation={[0.571, -0.763, 0.418]}>
                  <group name="Cube_1_instance_137" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4038" geometry={nodes.Cube_1_4038.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_39_2" position={[25.266, 0, -21.228]} rotation={[0.583, -0.782, 0.435]}>
                  <group name="Cube_1_instance_138" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4039" geometry={nodes.Cube_1_4039.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_3_2" position={[2.212, 0, -32.926]} rotation={[0.402, -0.062, 0.026]}>
                  <group name="Cube_1_instance_102" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4003" geometry={nodes.Cube_1_4003.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                ...
                <group name="Null_44_2" position={[27.477, 0, -18.276]} rotation={[0.654, -0.873, 0.531]}>
                  <group name="Cube_1_instance_143" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4044" geometry={nodes.Cube_1_4044.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_45_2" position={[27.879, 0, -17.657]} rotation={[0.671, -0.891, 0.553]}>
                  <group name="Cube_1_instance_144" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4045" geometry={nodes.Cube_1_4045.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_46_2" position={[28.266, 0, -17.03]} rotation={[0.688, -0.908, 0.575]}>
                  <group name="Cube_1_instance_145" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4046" geometry={nodes.Cube_1_4046.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_47_2" position={[28.64, 0, -16.393]} rotation={[0.707, -0.925, 0.599]}>
                  <group name="Cube_1_instance_146" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4047" geometry={nodes.Cube_1_4047.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                ...
                <group name="Null_53_2" position={[30.576, 0, -12.415]} rotation={[0.846, -1.021, 0.766]}>
                  <group name="Cube_1_instance_152" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4053" geometry={nodes.Cube_1_4053.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                ...
                <group name="Null_86_2" position={[30.975, 0, 11.383]} rotation={[2.253, -1.043, 2.326]}>
                  <group name="Cube_1_instance_185" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4086" geometry={nodes.Cube_1_4086.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_87_2" position={[30.712, 0, 12.073]} rotation={[2.282, -1.029, 2.359]}>
                  <group name="Cube_1_instance_186" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4087" geometry={nodes.Cube_1_4087.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_88_2" position={[30.435, 0, 12.756]} rotation={[2.309, -1.014, 2.391]}>
                  <group name="Cube_1_instance_187" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4088" geometry={nodes.Cube_1_4088.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_89_2" position={[30.142, 0, 13.434]} rotation={[2.335, -0.999, 2.422]}>
                  <group name="Cube_1_instance_188" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4089" geometry={nodes.Cube_1_4089.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_8_2" position={[5.872, 0, -32.473]} rotation={[0.407, -0.165, 0.071]}>
                  <group name="Cube_1_instance_107" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4008" geometry={nodes.Cube_1_4008.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_90_2" position={[29.834, 0, 14.104]} rotation={[2.36, -0.983, 2.451]}>
                  <group name="Cube_1_instance_189" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4090" geometry={nodes.Cube_1_4090.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_91_2" position={[29.511, 0, 14.768]} rotation={[2.383, -0.967, 2.479]}>
                  <group name="Cube_1_instance_190" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4091" geometry={nodes.Cube_1_4091.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_92_2" position={[29.174, 0, 15.424]} rotation={[2.404, -0.951, 2.505]}>
                  <group name="Cube_1_instance_191" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4092" geometry={nodes.Cube_1_4092.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_93_2" position={[28.822, 0, 16.072]} rotation={[2.425, -0.934, 2.53]}>
                  <group name="Cube_1_instance_192" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4093" geometry={nodes.Cube_1_4093.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_94_2" position={[28.455, 0, 16.713]} rotation={[2.444, -0.917, 2.555]}>
                  <group name="Cube_1_instance_193" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4094" geometry={nodes.Cube_1_4094.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_95_2" position={[28.074, 0, 17.345]} rotation={[2.462, -0.9, 2.578]}>
                  <group name="Cube_1_instance_194" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4095" geometry={nodes.Cube_1_4095.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_96_2" position={[27.679, 0, 17.968]} rotation={[2.479, -0.882, 2.6]}>
                  <group name="Cube_1_instance_195" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4096" geometry={nodes.Cube_1_4096.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_97_2" position={[27.271, 0, 18.582]} rotation={[2.496, -0.864, 2.621]}>
                  <group name="Cube_1_instance_196" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4097" geometry={nodes.Cube_1_4097.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_98_2" position={[26.849, 0, 19.187]} rotation={[2.511, -0.846, 2.641]}>
                  <group name="Cube_1_instance_197" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4098" geometry={nodes.Cube_1_4098.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_99_2" position={[26.413, 0, 19.783]} rotation={[2.525, -0.828, 2.661]}>
                  <group name="Cube_1_instance_198" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4099" geometry={nodes.Cube_1_4099.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
                <group name="Null_9_2" position={[6.596, 0, -32.334]} rotation={[0.409, -0.185, 0.08]}>
                  <group name="Cube_1_instance_108" position={[0, 0, 0.783]}>
                    <mesh name="Cube_1_4009" geometry={nodes.Cube_1_4009.geometry} material={materials['FLUO.003']} />
                  </group>
                </group>
              </group>
            </group>
            ...
            <group name="MAIN_COVER" position={[-8.531, 0.07, 0.011]} rotation={[-Math.PI / 2, 0, 0]}>
              <group name="BODY">
                <group name="BODYcercleAv">
                  <mesh name="COVER_medium001" geometry={nodes.COVER_medium001.geometry} material={materials['LIGHTS_BODY.001']} scale={[1.001, 1, 1.001]} />
                </group>
              ...
              <mesh name="LIGHTS_BODY" geometry={nodes.LIGHTS_BODY.geometry} material={materials['Mat.007']} />
              <mesh name="SCREEN" geometry={nodes.SCREEN.geometry} material={materials.visage1} position={[-53.306, 0.004, 0.248]} rotation={[-3.121, 0, Math.PI / 2]} scale={[34, 0.2, 34]} />
            </group>
            <group name="WINGS_SYM" position={[-8.531, 0.07, 0.011]} rotation={[-Math.PI / 2, 0, 0]}>
              <group name="WINGS" position={[29.528, -50.23, 0.092]} rotation={[0, 1.571, 0]}>
                <mesh name="WINGS_2" geometry={nodes.WINGS_2.geometry} material={materials['LIGHTS_BODY.002']} position={[0, 6.591, 14.529]} />
              </group>
            </group>
          </group>
        </group>
