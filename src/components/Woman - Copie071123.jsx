...
const setBloomLayer = (obj) => {
  if (obj && obj.isMesh && obj.name === "BLUE") {
...
// Call the function to set the bloom layer for "BLUE" objects
setBloomLayer(group.current); */
	
/*function Scene() {
  const size = useAspect(500, 500)
  return (
    <mesh scale={0.5}>
      <planeGeometry />
        <VideoMaterial url="Idle.mp4" />
    </mesh>
  )
}
...
videoTexture.minFilter = THREE.LinearFilter; // Set the minFilter as needed
...
if (materials) {
 ...
 envMapIntensity: 10.0,
	...
	materials.visage1.map.scale = 0.5;
	materials.visage1.map.color = new THREE.Color('red');

	
  materials['LIGHTS_BODY.002'] = new MeshStandardMaterial({
    metalness: 0.8, // Valeur de métal entre 0 (non métallique) et 1 (entièrement métallique)
    roughness: 0.02, // Valeur de rugosité entre 0 (lisse) et 1 (rugueux)
	color: '#ffffff',
  });
...
// Render the scene to the bloomComposer
renderer.autoClear = false;
bloomComposer.render();

finalComposer.addPass(bloomPass);
// Render the final scene with the mix effect
        ...
        <group name="FLUO" position={[0, 0.579, 0]} rotation={[-1.571, 1.55, 1.571]} scale={0.005}>
          <group name="AVATAR_ROTATION">
            ...
            <group name="MAIN_COVER" position={[-8.531, 0.07, 0.011]} rotation={[-Math.PI / 2, 0, 0]}>
              ...
              <mesh name="LIGHTS_BODY" geometry={nodes.LIGHTS_BODY.geometry} material={materials['Mat.006']} />
